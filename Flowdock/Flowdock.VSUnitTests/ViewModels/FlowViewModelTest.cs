...
namespace Flowdock.VSUnitTests.ViewModels {
    ...
    public class FlowViewModelTest {
        ...
        private ProgressServiceMoqaLate _progressService;
        private FlowStreamingConnectionMoqaLate _flowStreamConnection;
        private FlowViewModel _flowViewModel;

        private static MessageViewModel CreateDisplayableMessageViewModel(int userId = 0, string content = "a message") {
            return new MessageViewModel(CreateDisplayableMessage(userId, content), "some flow", null, new NavigationManagerMoqaLate());
        }

        private static Message CreateDisplayableMessage(int userId = 0, string content = "a message") {
            return new Message() {
                User = userId,
                Event = "message",
                Content = content
        ...
        [TestMethod]
        public void Activate_shows_progress_indicator() {
            _flowViewModel.Activate();

            Assert.AreEqual(1, _progressService.ShowTimesCalled());
            Assert.AreEqual(1, _progressService.HideTimesCalled());
        }
        ...
        [TestMethod]
        public void SendMessageToFlow_clears_out_NewMessage_afterwards() {
            string newMessage = "the new message";

            _flowViewModel.NewMessage = newMessage;
            _flowViewModel.SendMessageToFlow();

            Assert.IsTrue(string.IsNullOrEmpty(_flowViewModel.NewMessage));
        }

        [TestMethod]
        public void Trims_messages_as_they_arrive() {
            _flowViewModel.Activate();

            Assert.IsNotNull(_flowViewModel.Messages);
            _flowViewModel.Messages.Clear();

            for (var i = 0; i < FlowViewModel.MessageLimit; ++i) {
                _flowViewModel.Messages.Add(CreateDisplayableMessageViewModel());
                Assert.AreEqual(i + 1, _flowViewModel.Messages.Count);
            }

            _flowViewModel.Messages.Add(CreateDisplayableMessageViewModel());

            Assert.AreEqual(FlowViewModel.MessageLimit, _flowViewModel.Messages.Count);
        }
    }
}